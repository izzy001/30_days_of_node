/* Properties of errrors in node.js */
...
//error.message: it shows the description of the error. it is set using "new Error(msg)" property. 
...
//Error.stackTraceLimit: This property specifies number of stack frames captured by stack trace. it can be any valid JS number however 10 is the default value. If set to a non-number or negative number, then the stack trace will not return any frames.


/* Error propagation and interception in node.js */
//In node.js, we are having several mechanisms for error propagating and handle thse propagated errors while the applications is in its runnung state.]
...
/* Node.js methods follows a idiomatic pattern which is knows as Node.js style callback .
    Following this pattern , we pass the callback function as an argument to the method.
    When the execution of the operation is completed or an error is raised then the callback function is called with error object if there exists any error object otherwise the first argument is passed as null .  */
    ...
    try {
        ...
        console.error(err)
    } //This will not work because the method is asynchronous
    //The time by which the callback will be called , surrounding code already completed its execution and exited



    /* About errors */
    //In node.js applications, mostly all the errors belong to one of the metioned categories:
    ...
    //2. System errors: This type of errors are generated when the exception occurs during the run time of the program. 
        // They are generated when the application violated some constraints of operating system.
